...
ColorRGBA::ColorRGBA(const uint8_t *data) :
    r(static_cast<float>(data[0])),
    g(static_cast<float>(data[1])),
    b(static_cast<float>(data[2])),
    a(static_cast<float>(data[3])) {}

ColorRGB::ColorRGB(const uint8_t *data) :
    r(static_cast<float>(data[0])),
    g(static_cast<float>(data[1])),
    b(static_cast<float>(data[2])) {}


ColorRGB::ColorRGB(const ColorRGBA &input) :
...
ColorYIQ::ColorYIQ(const ColorRGB &input) :
    y(rgbToY(input)),
    i(rgbToI(input)),
    q(rgbToQ(input)) {}
